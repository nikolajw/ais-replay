...
sealed class Program
{
    ...
    {
        var app = new CommandApp<AisFileLoaderCommand>();
        app.Configure(config =>
        {
            config.SetApplicationName("AisFileLoader");
        });
        return await app.RunAsync(args);
    }
}

sealed class AisFileLoaderCommand : AsyncCommand<Options>
{
    public override async Task<int> ExecuteAsync(CommandContext context, Options options, CancellationToken cancellationToken)
